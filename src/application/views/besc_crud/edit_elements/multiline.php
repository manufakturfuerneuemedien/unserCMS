    	...
    	<?php if(count($formatting) > 0):?>
    		<div class="bc_col_multiline_formatting">
        		<?php foreach($formatting as $button):?>
        			<div class="bc_col_multiline_formatting_button"	tag="<?= $button?>"><?= strtoupper($button) ?></div>
        		<?php endforeach; ?>
    		</div>
    	<?php endif; ?>
    	<textarea name="col_<?= $db_name?>" style="<?php if(isset($height)) echo 'height: ' . $height . 'px;';?> <?php if(isset($width)) echo 'width: ' . $width . 'px;';?>"><?php if(isset($value)) echo $value;?></textarea>
	</div>
