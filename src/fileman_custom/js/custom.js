...
function FileSelected(file){
   ...
   * 
   */
	switch(RoxyUtils.GetUrlParam('module_type'))
	{
		case 'image':
			$(window.parent.document).find('#module_image_upload_preview').attr('src', file.fullPath);
			$(window.parent.document).find('#module_image_upload_preview').show();
			break;
		case 'sequence':
			$(window.parent.document).find('#module_sequence_upload_preview').attr('src', file.fullPath);
			$(window.parent.document).find('#module_sequence_upload_preview').show();
			break;		
		case 'gallery':
			var html = '<div class="gallery_item" fname="' + file.fullPath + '"><img src="' + file.fullPath + '"></div>';
			$(window.parent.document).find('#module_gallery_items').append(html);
			break;				
	}
